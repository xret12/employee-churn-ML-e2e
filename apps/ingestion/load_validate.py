...
class LoadValidate:
    ...
    def read_values_from_schema(self, schema_file):
        try:
            ...
            self.logger.exception('Exception raised when reading values from schema: %s' % e)
            raise e
    ...
    def validate_column_length(self, number_of_columns):
        try:
            ...
            self.logger.exception('OSError raised when validating column length')
            raise OSError
        
        except Exception as e:
            self.logger.exception('Exception raise when validating column length: %s' % e)
            raise e
    ...
    def validate_trainset(self):
        try:
            self.logger.info('Start of data load, validation, and transformation for trainset')
            # archive old files
            self.archive_old_files()
            # extract values from training schema
            column_names, number_of_columns = self.read_values_from_schema('schema_train')
            ...
            self.logger.exception('Encountered eror during data load, validation, and transformation for trainset')

    
    def validate_predictset(self):
        try:
            self.logger.info('Start of data load, validation, and transformation for predictset')
            # archive old files
            self.archive_old_files()
            # extract values from training schema
            column_names, number_of_columns = self.read_values_from_schema('schema_predict')
            # validate column length in the file
            self.validate_column_length(number_of_columns)
            # validate if a column has all values missing
            self.validate_missing_values()
            ...
            self.logger.exception('Encountered eror during data load, validation, and transformation for predictset')