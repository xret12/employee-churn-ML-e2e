...
class Preprocessor:
    ...
    def drop_columns(self, data, columns):
        ...
        try:
            ...
            self.useful_data = self.data.drop(labels=self.columns, axis=1) #drop the specified columns
    ...
    def is_null_present(self, data):
        try:
            ...
            if self.null_present:
                ...
                df_with_null['missing value counts'] = np.asarray(self.null_counts)
                df_with_null.to_csv(f'{self.data_path}_validation/null_values.csv')
    ...
    def encode_features(self, data):
        try:
            ...
            self.logger.exception('Exception raised while encoding features: %s' % e)
            raise e
    ...
    def split_features_label(self, data, label_name):
        ...
        try:
            ...
            self.logger.exception('Exception raised while splitting features and label: %s' % e)
            raise e
    ...
    def preprocess_trainset(self):
        try:
            ...
            self.logger.exception('Exception raised while preprocessing training data: %s' % e)
            raise e
    ...
    def preprocess_predictset(self):
        try:
            ...
            self.logger.exception('Exception raised while preprocessing prediction data: %s' % e)
            raise e